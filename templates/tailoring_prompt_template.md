...
8. NO client-name prefixes and NO engagement tags — weave the engagement name naturally INTO the sentence. Do NOT use leading labels ("[CLIENT] [PRODUCT]:") or em-dash tags ("[CLIENT] — "). Instead, make the client/engagement a natural grammatical part of the bullet (possessive or prepositional): "Migrated [CLIENT]'s [system] …", "… across [CLIENT]'s [platform] …", "Launched [product] at [CLIENT] …", "Increased [metric] by [X]% at [CLIENT] …". This applies to the client-engagement block only (single-employer entries need no client name). The insertion is a grammatical integration — the rest of the bullet's wording stays VERBATIM from the master profile; never use the integration to graft JD vocabulary onto the bullet. The engagement name counts toward the 25–35 word limit, so trim the bullet's own wording if it pushes past 35 — never drop the engagement name. Rationale: tags read as mechanical labels and make the consulting framing conspicuous; natural weaving reads as ordinary resume prose while still identifying each engagement.
